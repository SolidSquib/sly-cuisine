    ...
    private void Create2DMap()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
				...
				Collider2D[] overlap = Physics2D.OverlapCircleAll(new Vector2(x, y), Tilesize / 4);
				bool free = true;
                float maxZ = Mathf.Infinity;

				foreach (Collider2D col in overlap)
				{
					if (DisallowedTags.Contains(col.transform.tag))
					{
						Map[j, i] = new Node(j, i, y, ID, x, 0, false); //Non walkable tile!
						free = false;
					}
					else if (!IgnoreTags.Contains(col.transform.tag))
					{
                ...
                // walkable tile by default, we dont need a floor in a game without gravity
                if (free == true)
                {
